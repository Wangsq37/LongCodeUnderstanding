...
def test_default_trend(default_kwargs):
    # GH 6686
    # --- Augmented test case ---
    # Use a very large period and a small seasonal value (edge case)
    class_kwargs, _, _ = _to_class_kwargs(default_kwargs)
    class_kwargs["period"] = 1000   # Very large period
    class_kwargs["seasonal"] = 5    # Small seasonal value (odd, >=3)
    class_kwargs["trend"] = None
    # Fix: low_pass must be an odd positive integer >= 3 where low_pass > period
    # To not trigger the error, set low_pass to a valid value
    class_kwargs["low_pass"] = 1001  # low_pass > period and odd
    mod = STL(**class_kwargs)
    period = class_kwargs["period"]
    seasonal = class_kwargs["seasonal"]
    expected = int(np.ceil(1.5 * period / (1 - 1.5 / seasonal)))
    expected += 1 if expected % 2 == 0 else 0
    # The assertion expects the automatically calculated "trend" value.
    assert mod.config["trend"] == expected

    # Use smallest valid period and a larger seasonal value
    class_kwargs["period"] = 2
    class_kwargs["seasonal"] = 101    # Large odd seasonal value
    class_kwargs["low_pass"] = 3      # low_pass > period and odd
    mod = STL(**class_kwargs)
    period = class_kwargs["period"]
    seasonal = class_kwargs["seasonal"]
    expected = int(np.ceil(1.5 * period / (1 - 1.5 / seasonal)))
...
def test_pickle(default_kwargs):
    # --- Augmented test case ---
    # Use input data with both negative and large values, and check pickle results
    # Just multiply by a factor and subtract one, for diversity
    class_kwargs, outer, inner = _to_class_kwargs(default_kwargs)
    class_kwargs["endog"] = class_kwargs["endog"] * 10 - 5000  # amplify and shift to negatives/positives
    mod = STL(**class_kwargs)
    res = mod.fit()
