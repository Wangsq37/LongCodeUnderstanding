...
class TestTuckeyHSD2(CheckTuckeyHSDMixin):
    ...
    def test_table_names_custom_group_order(self):
        # AUGMENTED TEST CASE: Use custom/expanded group names & orders, extra/large and edge-case data
        # using larger/float and edge-case values, and group names with unusual bytes
        # We'll create a new test dataframe and run the logic the same way

        # new "groups" with bytes and special characters
        group_order = [b'z-group', b'-alpha', b'', b'LONGGGGGGGGGGROUP']
        # custom test endog and group
        # purposely create float/large/int/negative/zero/empty-string feature
        test_endog = np.array(
            [100.0, -25.5, 0.0, 1e8, 0.001, 123456, 99999, -0.0005, 99999, 0.]
        )
        test_groups = np.array([
            b'LONGGGGGGGGGGROUP', b'LONGGGGGGGGGGROUP', b'-alpha',
            b'z-group', b'-alpha', b'z-group', b'', b'', b'z-group', b''  # 4 groups, with 3,2,3,2 members
        ])
        ...
        # The expected order: all pairs from custom group_order, in pairwise order
        expected_order = [
            (b'z-group', b'-alpha'),
            (b'z-group', b''),
            (b'z-group', b'LONGGGGGGGGGGROUP'),
            (b'-alpha', b''),
            (b'-alpha', b'LONGGGGGGGGGGROUP'),
            (b'', b'LONGGGGGGGGGGROUP')
        ]
        ...
        # the order of pairwise stats in the frame matches group_c and group_t (target, control)
        group_t = [b'-alpha', b'', b'LONGGGGGGGGGGROUP',
                   b'', b'LONGGGGGGGGGGROUP', b'LONGGGGGGGGGGROUP']
        group_c = [b'z-group', b'z-group', b'z-group',
                   b'-alpha', b'-alpha', b'']
