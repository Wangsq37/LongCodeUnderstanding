...
class CompareMixin:
    ...
    # AUGMENTED TEST: test_df
    def test_df(self):
        # Use a more diverse edge case: k_extra=2, nobs=10, k_vars=4
        class DummyRes:
            def __init__(self):
                self.model = type('DummyModel', (), {})()
                # exog: shape (10, 4)
                self.model.exog = np.eye(10, 4)
        res = DummyRes()
        res.df_resid = 10 - 4 - 2
        res.df_model = 4 - 1
        res.params = np.zeros(4 + 2)
        k_extra = 2
...
class TestPoissonZi(CompareMixin):
    ...
    # AUGMENTED TEST: test_df
    def test_df(self):
        # Edge case: k_extra = 5, nobs = 20, k_vars = 8, long param vector
        class DummyRes:
            def __init__(self):
                self.model = type('DummyModel', (), {})()
                # exog shape (20, 8)
                self.model.exog = np.ones((20, 8))
        res = DummyRes()
        res.df_resid = 20 - 8 - 5
        res.df_model = 8 - 1
        res.params = np.zeros(8 + 5)
        k_extra = 5
        nobs, k_vars = res.model.exog.shape
        assert res.df_resid == nobs - k_vars - k_extra
        assert res.df_model == k_vars - 1  # -1 for constant
...
# Standalone, non-mixin edge case augmented test
def test_df():
    # Assign edge-case values for a generic Poisson model result mock
    class DummyRes:
        def __init__(self):
            self.model = type('DummyModel', (), {})()
            # exog shape (3,2)
