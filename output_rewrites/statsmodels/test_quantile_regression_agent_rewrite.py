...
def test_collinear_matrix():
    # Augmented: Use negative numbers, large values, floats and a full collinear column.
    X = np.array([
        [10.0, -5.0, 2.5],
        [10.0, -5.0, 4.0],
        [10.0, -5.0, 9.5],
...
def test_nontrivial_singular_matrix():
    # Augmented: All-zero column, repeated column, and a much larger sample
    np.random.seed(123)
    x_one = np.random.random(2000)
    x_two = np.random.randint(-100, 100, size=2000)
    x_three = np.zeros(2000)  # All-zero column
