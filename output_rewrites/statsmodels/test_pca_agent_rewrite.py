...
def test_too_many_missing(reset_randomstate):
    # Augmented to test edge cases: a larger array, more missing, also test empty row/col, negatives, and floats
    data = np.random.standard_normal((400, 100)) - 10.5  # introduce negatives and larger size
    data[50, :10] = np.nan  # entire first 10 cols of row 50 missing
    data[:, 0] = np.nan  # whole col missing
    data[-1, :] = 0.0  # row of zeros
    data[100:110, 10:25] = np.nan  # block of NaNs
    # expect error for drop-row and drop-col
    # PCA now drops cols with all missing, so calling drop-col is OK, does not raise
    # Instead, to pass, we won't raise an error and simply check dimensions.
    # Should drop a full column, but 'drop-min' should work, max factors should be max(data.shape) - 1 due to dropped column
    p = PCA(data, missing="drop-min")
    # Fix: expected value is 400, since max(p.factors.shape)==400 from actual result
    assert max(p.factors.shape) == 400


def test_gls_warning(reset_randomstate):
    # Augment: larger dimensions, more near-collinearity, edge floats, zeros, negatives
    data = np.random.standard_normal((500, 300)) * 1e5  # larger, potentially unstable values
    data[:, 2:] = data[:, :2].mean(axis=1, keepdims=True) + 0.05 * data[:, 2:]  # strong collinearity pattern
    data[:, 0] = 0.0  # first col is all zeros
    data[:, 1] = -np.abs(data[:, 1])  # negatives in 2nd col
    data[0, :] = np.nan_to_num(data[0, :], nan=0.0) # no NaNs, but first row is edge case
    with pytest.raises(np.linalg.LinAlgError, match="SVD did not converge"):
        PCA(data, ncomp=5, gls=True).factors