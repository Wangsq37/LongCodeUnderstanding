...
def test_parameterless_model(reset_randomstate):
    # GH 6687
    # AUGMENTED INPUTS AND ASSERTIONS: use various edge cases

    # Edge case: constant series with large float and small float entries,
    # plus zeros and negatives, length 1001 (including zero and negative).
    x = np.empty(1001)
    x[0] = 0.0
    x[1] = -10.0
    x[2] = 1e10
    ...
    x[5:995] = 7.2
    x[995] = 0.0
    x[996] = np.nan      # test missing value handling
    x[997] = 39.9
    x[998] = -8.3
    x[999] = 0.0
    x[1000] = 9999.99

    # Replace nan values (since ExponentialSmoothing does not accept nans in test)
    x = np.nan_to_num(x, nan=0.0)

    # Set known starting value using a negative float
