...
class TestLagmat:
    ...
    def test_range_index_columns(self):
        # GH 8377
        # Augment: Use more columns and test edge cases: negative, zero, and float values.
        df = pd.DataFrame(
            {
                0: np.zeros(50, dtype=float),
                1: np.arange(-50, 0, dtype=float),
                2: np.linspace(-100, 100, 50),
                3: np.full(50, 3.14),
            }
        )
        df.columns = pd.RangeIndex(4)
        result = stattools.lagmat(df, maxlag=3, use_pandas=True)
        # The shape should be (50, 12) because lags = 3, 4 columns.
