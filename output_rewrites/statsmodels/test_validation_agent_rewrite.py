...
class TestArrayLike:
    ...
    def test_right_squeeze_and_pad(self):
        # Change test data to various edge cases and checks
        # Case 1: Minimal size, already 3d
        data = np.empty((1, 1, 1))
        a = array_like(data, "a", ndim=3)
        assert a.shape == (1, 1, 1)
        # Case 2: 1D data, larger size, should pad
        data = np.empty(5)
        a = array_like(data, "a", ndim=3)
        assert a.shape == (5, 1, 1)
        # Case 3: 2D, non-square shape
        data = np.empty((7, 3))
        a = array_like(data, "a", ndim=3)
        assert a.shape == (7, 3, 1)
        # Case 4: Higher # dimensions, trigger ValueError
...
def test_right_squeeze():
    # New test inputs: empty (1, 0, 1) array, should squeeze trailing 1
    x = np.empty((1, 0, 1))
    with pytest.raises(ValueError):
        _right_squeeze(x)

    # Trailing axes with length-1 in odd places, squeeze last 1's only
    x = np.empty((2, 3, 4, 1))
    y = _right_squeeze(x)
    assert y.shape == (2, 3, 4)

    # No trailing 1, shape should remain
    x = np.empty((7, 2, 2))
    y = _right_squeeze(x)
    assert y.shape == (7, 2, 2)

    # Several trailing ones, squeeze all trailing 1s
...
def test_wrap_pandas_append():
    # Try a non-default, non-string name, and long append string
    a = gen_data(1, True)
    a.name = 42
    b = gen_data(1, False)
...
def test_wrap_pandas_append_non_string():
    # GH 6826 - Columns as floats, name as None, append as int
    a = gen_data(1, True)
    a.name = None
    b = gen_data(1, False)
