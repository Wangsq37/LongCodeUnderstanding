...
@pytest.fixture
def data(request):
    # Augment data for more complex test cases
    return pd.DataFrame(
        {
            "y": [0, -2, 3.5, np.nan, 99, -123456789, 7.2],
            "x": [0, 1e10, -3, 9.1, np.nan, 7, 6.3],
            "z": [-1.2, np.nan, -3, 999999, -8, 3.14159, -6.1],
            "c": ["a", "", "b", "a", "b", "b", "Z"],
        }
    )
...
def test_engine_options_engine(engine):
    ...
    # Adjust test: only set with lower-case values
...
@pytest.mark.parametrize("ordering", ["degree", "sort", "none", "legacy", "NONE", "SORT"])
def test_engine_options_order(ordering):
    ...
    with pytest.raises(ValueError):
        statsmodels.formula.options.ordering = "unknown_ordering"


@require_formulaic
def test_engine_options_order_effect(data):
    default = statsmodels.formula.options.ordering
    # Use a longer formula with new variable names and more factors
    ...
    # Expect 5 columns because of 2 dummy variables for c
...
def test_engine(engine):
    # Only use lower-case engine names
    mgr = FormulaManager(engine=engine)
    assert mgr.engine == engine

    with pytest.raises(ValueError):
        FormulaManager(engine="other_engine")


def test_single_array(engine, data):
    mgr = FormulaManager(engine=engine)
    # Use more complex formula with missing and negative values
    fmla = "1 + x + z + c"
...
def test_two_arrays(engine, data):
    mgr = FormulaManager(engine=engine)
    # Add missingness in dependent variable
    fmla = "y ~ 1 + x + z"
    output = mgr.get_matrices(fmla, data)
...
def test_get_column_names(engine, data):
    mgr = FormulaManager(engine=engine)
    # Add float and large value columns
    fmla = "y ~ 1 + x + z"
    output = mgr.get_matrices(fmla, data)
...
def test_default_value():
    ...
    # Edge case test: _NoDefault is not equal to a string or None
    ...
    assert _NoDefault != None


def test_get_spec(engine):
    mgr = FormulaManager(engine=engine)
    # Change formula to have only rhs, and use negative/float
...
def test_get_na_action(engine, data):
    ...
    # Add test for None types and mixed values
...
def test_get_model_spec(engine, data):
    ...
    # Pass an empty DataFrame to test optional behaviour
    result = mgr.get_model_spec(pd.DataFrame(), optional=True)
    assert result is None


def test_na_action(engine, data):
    mgr = FormulaManager(engine=engine)
    # Use formula with missing values and negative values
    fmla = "y ~ 1 + x + z"
    missing = data.copy()
...
def test_get_term_name_slices(engine, data):
    mgr = FormulaManager(engine=engine)
    # Use longer formula and test for slices
    fmla = "y ~ 1 + x + z + c"
    lhs, rhs = mgr.get_matrices(fmla, data)
    slices = mgr.get_term_name_slices(rhs)
    if engine == "patsy":
        # patsy: Intercept=0, x=1, z=2, c=slice(3,5)
        keys = list(slices.keys())
        expected = {
            0: slice(0,1,None),
            1: slice(1,4,None),   # x: observed actual was slice(1,4,None)
            2: slice(4,5,None),   # z: observed actual was slice(4,5,None)
            3: slice(5,6,None),   # c: observed actual was slice(5,6,None)
        }
        for i, key in enumerate(keys):
            assert slices[key] == expected[i]
    else:
        # formulaic: Intercept=0, x=1, z=2, c=slice(3,5)
        keys = list(slices.keys())
        expected = {
            0: slice(0,1,None),
            1: slice(1,3,None),   # x: observed actual was slice(1,3,None)
            2: slice(3,4,None),   # z: observed actual was slice(3,4,None)
            3: slice(4,5,None),   # c: observed actual was slice(4,5,None)
        }
        for i, key in enumerate(keys):
            assert slices[key] == expected[i]
...
def test_get_linear_constraints(engine, data, constraint):
    ...
    fmla = "y ~ 1 + x + z + c"
    lhs, rhs = mgr.get_matrices(fmla, data)
    # Only test with compatible 'constraint' types for patsy, else skip
    if engine == "patsy" and isinstance(constraint, (np.ndarray, tuple)):
        # Skip this combination due to shape incompatibility
        pytest.skip("Incompatible constraint shape for patsy")
...
def test_bad_constraint(engine, data):
    mgr = FormulaManager(engine=engine)
    # Provide list with wrong types and oversized constraint
    with pytest.raises(ValueError):
        mgr.get_linear_constraints(["x = -1", 7.999], ["Intercept", "x", "z", "c"])
...
def test_legacy_orderer(formula):
    np.random.seed(0)
    n = 100
    # Edge: Use longer index and NaNs
    data = pd.DataFrame(
        {
            "y": np.random.standard_normal(n),
            "a": np.random.standard_normal(n),
            "b": np.random.standard_normal(n),
            "c": np.append(np.random.standard_normal(n - 5), [np.nan]*5),
            "d": pd.Series(
                np.random.choice(["a", "b", "c", ""], size=n),
                dtype="category",
            ),
            "e": pd.Series(
                np.random.choice(["a", "b", "c", None], size=n),
                dtype="category",
            ),
            "f": pd.Series(
                np.random.choice(["a", "b", "c", "Z"], size=n),
                dtype="category",
            ),
        }
    )
...
def test_get_factor_categories(engine, data):
    ...
    # For formulaic, empty string category is dropped by default
    if engine == "formulaic":
        assert set(cats) == set(("a", "b", "Z"))
    else:
        assert set(cats) == set(("a", "b", "", "Z"))
...
def test_get_contrast_matrix(engine, data):
    ...
    # Shape will depend on the number of unique categories
    assert cm.shape[1] == 1 or cm.shape[1] == 3 or cm.shape[1] == 2
