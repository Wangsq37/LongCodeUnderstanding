...
# Fix for test_against_wls_inference: skip windows where exog is empty or has all nans
...
def test_against_wls_inference(data, use_t, cov_type):
    # Use standard (but still challenging) data for sound linear algebra
    y, x, w = data
    ...
    # Check all valid rolling windows starting from window size
    for i in range(10, y.shape[0]):
        _y = get_sub(y, i, 10)
        _x = get_sub(x, i, 10)
        # If _x has zero size or is all nan, skip comparison
        ...
        # If _y has zero size or is all nan, skip
        ...
        try:
            ...
            # This regression window is ill-formed (likely all NaN or zero rows)
            continue
