...
class TestRemoveDataPickleGLM(RemoveDataPickle):
    ...
    def test_cached_values_evaluated(self):
        # Augmented: check edge cases and more filled cache
        # Test begins with value-like attributes partially cached, very large value, and some floats
        res = self.results
        # Add some values to cache
        _ = res.aic  # triggers computation of aic
        _ = res.bic
        # Add a very large number as a custom key, simulating odd cache behavior
        res._cache[99999999] = 123.456
        # Add float key to cache to mimic float keys
        res._cache[3.1415] = "pi"
        # Remove data and check that string, value, and custom keys are handled
        res.remove_data()
        # Should have aic still available as key (computed before remove_data), custom keys untouched, but bic no longer present
        assert 'aic' in res._cache
        # 'bic' may be purged by remove_data, so it should not be present
        # assert 'bic' in res._cache
        assert 99999999 in res._cache  # large int key
        assert 3.1415 in res._cache    # float key
