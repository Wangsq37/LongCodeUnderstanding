...
class TestTEffects():
    ...
    def test_aux(self):
        # Augmented test: check prediction on an explicit, edge case data
        prob = res_probit.predict(exog=pd.DataFrame({
            "mmarried_": [0, 1, 0, 1],
            "mage": [0, 25, 40, -10],      # includes 0, normal, high, negative
            "mage2": [0, 625, 1600, 100],  # squares of above
            "fbaby_": [1, 0, 1, 0],        # try 0 and 1
            "medu": [0, 18, 30, np.nan]    # includes extremes, NaN
        }))
        # For new (likely unseen) data, prediction shape checks number of inputs.
        # We expect the shape to match number of rows of input, i.e., (4,)
        assert prob.shape == (4,)
