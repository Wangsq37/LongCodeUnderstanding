...
class TestArea:
    ...
    def test_unfilled(self):
        ...
        # Augmenting test data: challenging values, including float, negative, zero and a very large value for x/y,
        # and a different color
        x, y = [0, -5.5, 1e6, 3.14], [0, 5, -2.72, 0]
        c = "red"
        p = Plot(x=x, y=y).add(Area(fill=False, color=c)).plot()
        ax = p._figure.axes[0]
        poly = ax.patches[0]
        # Guess: unfilled area (fill=False) returns facecolor with alpha=0, so we expect transparent
