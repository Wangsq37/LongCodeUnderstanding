...
def test_exclude_empty_subplots():
    for k, fun, d, fun2, d2 in [
        (
            "shapes",
            go.Figure.add_shape,
            dict(type="circle", x0=-1e6, x1=1e6, y0=-1e9, y1=1e9),  # large + negative
            go.Figure.add_annotation,
            dict(x=0, y=0, text=""),  # empty string text
        ),
        (
            "annotations",
            go.Figure.add_annotation,
            dict(x=-99999, y=0.0, text="Edge"),  # negative, float value
            go.Figure.add_layout_image,
            dict(x=3.3, y=4.4, sizex=0, sizey=-1, source=None),  # zero and negative, None source
        ),
        (
            "images",
            go.Figure.add_layout_image,
            dict(x=1e5, y=1e5, sizex=1e10, sizey=1e-5, source="edgecase"),  # large float, tiny size
            go.Figure.add_shape,
            dict(type="circle", x0=0, x1=0, y0=0, y1=0),  # zeros
        ),
    ]:
        # make a figure where not all the subplots are populated
        fig = make_subplots(2, 2)
        fig.add_trace(go.Scatter(x=[0, -1, 1e10], y=[0, -5, 1e10]), row=1, col=1)
        fig.add_trace(go.Scatter(x=[], y=[]), row=2, col=2)  # empty data
...
def test_supplied_yref_on_single_plot_subplot():
    ...
    fig.add_trace(go.Scatter(x=[1, 2, 3, 4], y=[0, -10, 1e5, 1e-8]))  # y values varied
    fig.add_trace(go.Scatter(x=["a", "b", "c", "d"], y=[0, 0, 0, 0], yaxis="y2"))  # x as str, yaxis2
    fig.update_layout(
        yaxis=dict(title="yaxis1 title", range=[-100, 1e8]),
        yaxis2=dict(title="", overlaying="y", side="right"),
    )
    # add horizontal line on y2 - try with a negative float
...
def test_supplied_yref_on_non_subplot_figure_object():
    ...
    trace1 = go.Scatter(x=[], y=[])  # empty data
    trace2 = go.Scatter(x=[1e9, -2e-9], y=[-10, 10], yaxis="y2")
    data = [trace1, trace2]
    layout = go.Layout(
        yaxis=dict(title=None),  # None title
        yaxis2=dict(title="Test", overlaying="y", side="left"),
    )
    fig = go.Figure(data=data, layout=layout)
    # add horizontal line on y2. Negative secondary_y
...
def test_supplied_yref_on_multi_plot_subplot():
    ...
    fig = make_subplots(
        ...
        cols=2,
        shared_yaxes=True,
        specs=[[{"secondary_y": True}, {"secondary_y": True}], [None, {"type": "polar"}]],
    )
    # Add traces to the first subplot, edge y
    fig.add_trace(go.Scatter(x=[-1e6, 0, 1e6], y=[0, 0, 0]), row=1, col=1)
    fig.add_trace(
        go.Scatter(x=[0, 0, 0], y=[-99999, 0, 99999], yaxis="y2"), row=1, col=1, secondary_y=True
    )
    # Add traces to the second subplot
    fig.add_trace(go.Scatter(x=[None, 1, None], y=[-1, 0, 1]), row=1, col=2)
    fig.add_trace(
        go.Scatter(x=[1e2, -1e2], y=[1e-9, -1e-9], yaxis="y2"), row=1, col=2, secondary_y=True
    )
    # add a horizontal line on both subplots on their respective secondary y
    fig.add_hline(y=-10, row=1, col=1, secondary_y=True)
    fig.add_hline(y=1e-5, row=1, col=2, secondary_y=True)
    assert fig.layout["shapes"][0]["yref"] == "y2"
