...
def test_add_shape_no_annotation(multi_plot_fixture):
    # Augmented: use negative and float values, edge cases for y; also test with zero
    multi_plot_fixture.add_hline(y=0.0, row="all", col="all")
    multi_plot_fixture.add_hline(y=-10000, row="all", col="all")
    multi_plot_fixture.add_hline(y=1e12, row="all", col="all")
    # None of these should add annotations, only shapes
    assert len(multi_plot_fixture.layout.annotations) == 0
    assert len(multi_plot_fixture.layout.shapes) == 12  # 3 hlines * 4 subplots
