...
class TestJSONEncoder(TestCase):
    ...
    def test_figure_json_encoding(self):
        # Augmented test data:
        df = pd.DataFrame(columns=["Value"], data=[0, -1, 1e10, 3.14159])
        # types: 0 (zero), -1 (negative int), large float, pi (float)

        s1 = Scatter3d(
            x=[0, -999, 9999999999],
            y=np.array([0.0, -999.99, np_nan(), dt(2024, 1, 1)]),
            z=[
                "",
                "Z",
                dt(2024, 1, 1),
                dt(2024, 1, 1, 23, 59, 59, 999999),
            ],
        )
        ...
        assert (
            js1
            == '{"type": "scatter3d", "x": [0, -999, 9999999999], '
            '"y": [0.0, -999.99, null, "2024-01-01T00:00:00"], '
            '"z": ["", "Z", "2024-01-01T00:00:00", '
