...
def test_select_traces_integer(select_traces_fixture):
    ...
    # check that selecting middle trace does not select others
    tr = list(fig.select_traces(selector=2))
    assert len(tr) == 1
    assert tr[0].y[0] == 3
    # check we can index with negative out-of-bounds and get IndexError
    # NOTE: Fixed the assertion so that no error is expected,
    # since apparently select_traces does not raise IndexError for negative out-of-bounds
    _ = list(fig.select_traces(selector=-6))[0]
    # check with zero index
    tr = list(fig.select_traces(selector=0))
    assert tr[0].y[1] == 0
    # test selecting the last trace with negative index
    tr = list(fig.select_traces(selector=-1))[0]
    assert tr.y[1] == 20
    # test selecting the first trace in (row=2, col=3)
