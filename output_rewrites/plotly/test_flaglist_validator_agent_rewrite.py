...
def test_acceptance(flaglist, validator):
    # Edge case: Accept different data type, pass in empty string and test long combination
    special_flaglist = "lines+markers+text"
    empty_flaglist = ""
    mixed_case_flaglist = "LINES+markers+Text"
    numeric_flaglist = "lines+3"
    # Pick new test case: mixed_case_flaglist should be rejected, empty should be rejected,
    # but special_flaglist is always valid (all flags)
    assert validator.validate_coerce(special_flaglist) == special_flaglist
...
@pytest.mark.parametrize(
    "in_val,coerce_val",
    [
        ("      markers   ", "markers"),                   # Lots of whitespace
        ("markers   +   text", "markers+text"),            # Internal whitespace
        ("lines , markers , text", "lines+markers+text"),  # Comma separated, all flags
        ("lines ,", ValueError),                           # Trailing comma -- should raise error!
        ("lines,,,markers", ValueError),                   # Multiple commas -- should raise error!
        ("LINES + MARKERS", ValueError),                   # Uppercase input -- not valid, should raise error!
    ],
)
...
def test_acceptance_extra(extra, validator_extra):
    # Edge case: True, False, numbers, and long string (EXTRAS includes 3)
    # Also try 'none'
    new_extra = 3
    assert validator_extra.validate_coerce(new_extra) == new_extra
...
@pytest.mark.parametrize(
    "in_val,coerce_val",
    [
        ("true  ", ValueError),    # Extratype edge case, bool True as string -- not accepted!
        (" 3 ", ValueError),          # Whitespace with numeric extra -- not accepted!
        ("False", ValueError),    # String bool 'False' -- not accepted!
        ("none    ", "none"), # Existing
        ("all  ", "all"),
        ("NONE", ValueError),   # Uppercase version, will not pass!
    ],
)
...
def test_acceptance_aok_scalar_flaglist(flaglist, validator_extra_aok):
    # Augment: Use a large combination, and combine all flags
    all_flags = "markers+text+lines"
    assert validator_extra_aok.validate_coerce(all_flags) == all_flags


def test_acceptance_aok_scalar_extra(extra, validator_extra_aok):
    # Augment: Use boolean extra True, which is in EXTRAS
    new_extra = True
    assert validator_extra_aok.validate_coerce(new_extra) == new_extra
...
def test_acceptance_aok_list_flaglist(val, validator_extra_aok):
    assert np.array_equal(
        validator_extra_aok.validate_coerce(val), np.array(val, dtype="unicode")
    )
...
def test_coercion_aok(in_val, expected, validator_extra_aok):
    ...
    if isinstance(in_val, (list, tuple)):
        assert np.array_equal(coerce_val, np.array(expected, dtype="unicode"))
        assert validator_extra_aok.present(coerce_val) == tuple(expected)
    else:
