...
# Augmented test data for graph construction for more edge cases:
graphs = [
    example_graph_random(feature_size=8, n_nodes=7),
    example_graph_random(feature_size=8, n_nodes=2),  # minimal graph for edge case
    # Remove the problematic case with n_nodes=1 -- that triggers error in collection
    # example_graph_random(feature_size=8, n_nodes=1),
]
...
def test_generator_init():
    # Use graphs with:
    # multiple nodes, and a feature_size much larger
...
def test_generator_different_feature_numbers():
    graphs_diff_num_features = [
        example_graph_random(feature_size=1, n_nodes=6),
        example_graph_random(feature_size=3, n_nodes=3),
    ]

    with pytest.raises(
        ValueError,
        match="graphs: expected node features for all graph to have same dimensions,.*1.*3",
    ):
...
def test_generator_init_nx_graph():
    graphs_nx = [
        example_graph_random(feature_size=8, n_nodes=7, is_directed=False),
        example_graph_random(
            feature_size=8, n_nodes=2, is_directed=False
        ).to_networkx(),
    ]
...
def test_generator_empty():
    graphs_local = [
        example_graph_random(feature_size=3, n_nodes=2),
        example_graph_random(feature_size=3, node_types=0, edge_types=0),
    ]
    ...
    with pytest.raises(
        ...
        generator = PaddedGraphGenerator(graphs=graphs_local)

    generator = PaddedGraphGenerator(graphs=graphs_local[:1])
    with pytest.raises(
        ValueError,
        match="graphs: expected every graph to be non-empty, found graph with no nodes",
    ):
        seq = generator.flow(graphs_local)
...
def test_generator_flow_no_targets():
    ...
    assert len(seq) == 1  # only one batch (since one graph)

    # First batch: graph[1] (n_nodes=2)
...
def test_generator_flow_check_padding():
    # Testing graphs with large difference in nodes
    generator = PaddedGraphGenerator(graphs=graphs)
    ...
    # graph[0] has 7 nodes, graph[1] has 2 nodes
    batch = seq[0]
    ...
    # Masks: sum is either 7 (all valid) or 2 (minimal graph)
    for mask in batch[0][1]:
        assert np.sum(mask) == 7 or np.sum(mask) == 2
...
def test_generator_flow_with_targets():
    ...
    seq = generator.flow(graphs=[0, 1], targets=np.array([-1, 99999]), batch_size=1)
    assert isinstance(seq, PaddedGraphSequence)
...
def test_generator_adj_normalisation(symmetric_normalization, weighted):
    ...
    graph = example_graph(feature_size=9, edge_weights=True)
...
def test_generator_flow_shuffle():
    ...
    # shuffle = False
    seq = generator.flow(graphs=[0, 1], batch_size=1, shuffle=False)
    ...
    seq = generator.flow(graphs=[0, 1], batch_size=1, shuffle=True, seed=0)
    batches = get_batches(seq)
...
def test_generator_flow_StellarGraphs():
    generator = PaddedGraphGenerator(graphs=graphs)
    graph_ilocs = [1, 0]  # changed order for edge case
