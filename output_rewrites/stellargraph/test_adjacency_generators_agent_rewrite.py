...
def test_init(barbell):
    ...
    # Use a larger num_powers, and ensure edge cases for adjacency shapes
...
def test_bad_init(barbell):
    ...
    with pytest.raises(TypeError):
        generator = AdjacencyPowerGenerator(None, num_powers=2)  # only change num_powers

    with pytest.raises(ValueError, match="num_powers: expected.*found -100"):
        generator = AdjacencyPowerGenerator(barbell, num_powers=-100)

    with pytest.raises(TypeError, match="num_powers: expected.*found float"):
        generator = AdjacencyPowerGenerator(barbell, num_powers=0.5)
...
def test_bad_flow(barbell):
    ...
    with pytest.raises(TypeError, match="batch_size: expected.*found float"):
        generator.flow(2.5, num_parallel_calls=1)

    with pytest.raises(ValueError, match="num_parallel_calls: expected.*found 0"):
        generator.flow(3, num_parallel_calls=0)

    with pytest.raises(TypeError, match="num_parallel_calls: expected.*found float"):
        generator.flow(3, num_parallel_calls=2.5)


@pytest.mark.parametrize("batch_size", [0, 2, 50])
def test_flow_batch_size(barbell, batch_size):
    ...
    for x, y in generator.flow(batch_size=batch_size if batch_size > 0 else 1).take(1):
        ...
        assert y.shape == (
            batch_size if batch_size > 0 else 1,
            1,
            len(barbell.nodes()),
        )
        assert x[1].shape == (
...
@pytest.mark.parametrize("num_powers", [0, 3, 16])
def test_flow_batch_size(barbell, num_powers):
    ...
    for x, y in generator.flow(batch_size=batch_size).take(1):
        ...
        assert x[1].shape == (batch_size, npw, len(barbell.nodes()))
