...
class TestIndepVarComp(unittest.TestCase):
    ...
    def test_add_output_retval(self):
        # check metadata for more varied test data
        expected = {
            'val': -123.456,
            'shape': (1,),
            'size': 1,
            'units': 'kg',
            'desc': 'test negative float',
        ...
        class IDVComp(om.IndepVarComp):
            def setup(self):
                # add_output with negative float, units, description
                ...
                # add_discrete_output with large int, description
