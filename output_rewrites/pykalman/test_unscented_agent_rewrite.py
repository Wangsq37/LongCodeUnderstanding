...
def test_unscented_sample():
    ...
    # Augment the test by using zero samples and very large samples
    # test with zero samples (edge case) and with a larger number than before
    for n_samples in [0, 1000]:
        x, z = kf.sample(n_samples)
        assert x.shape == (n_samples, 2)
        assert z.shape == (n_samples, 1)
    # Test with n_samples=5 for coverage with a small number
    x, z = kf.sample(5)
    assert x.shape == (5, 2)
    assert z.shape == (5, 1)
...
def test_additive_sample():
    ...
    # Augment: test zero, negative, large, and float number of samples
    # Zero samples
    x, z = kf.sample(0)
    assert x.shape == (0, 2)
    assert z.shape == (0, 1)
