...
def df2():
    ...
    # Augment: Use floats, NaNs, negative, large values, and empty geometries for edge cases
    return GeoDataFrame(
        {
            "geometry": GeoSeries(
                [Point(x * 1.5, x * -2.5) if x != 2 else Point() for x in range(3)]
            ),
            "geometry2": GeoSeries(
                [Point(-1000, 1e10), Point(0, 0), Point(np.nan, np.nan)]
            ),
            "geometry3": GeoSeries([Point(x, x + 1000) for x in range(3)]),
            "value": [0, -999, 1e12],
            "value_nan": [np.nan, "", None],
        }
    )
...
def test_loc_add_row(geom_name, nybb_filename):
    ...
    # Augment: Add a row with large negative values and a None geometry
    nybb.loc[100] = [-99999999, None]
    # We also retain the original test for completeness by including the original added row at 5
    nybb.loc[5] = [6.7, nybb.geometry.iloc[0]]  # use float for value
...
# Augment: Provide new column combinations and edge cases
...
def test_constructor_sliced_row_slices(df2, column_set):
    ...
    if "geometry" in column_set:
        ...
        # The augmented df2 now Point(0*1.5, 0*-2.5) for geometry col at loc[0]
        assert res.geometry == Point(0.0, 0.0)
    if "geometry2" in column_set:
        # geometry2 at loc[0] is Point(-1000, 1e10)
        assert res.geometry2 == Point(-1000, 1e10)
    if "geometry3" in column_set:
        assert res.geometry3 == Point(0, 1000)
    if "value" in column_set:
        assert res.value == 0
    if "value_nan" in column_set:
        assert (pd.isnull(res.value_nan) or res.value_nan == "" or res.value_nan is None)
