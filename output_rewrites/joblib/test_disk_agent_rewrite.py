...
@parametrize(
    ...
    [
        ("0M", 0),                            # Edge: zero size
        ("5.5K", int(5.5 * 1024)),            # Edge: float kilobytes
        ("1024T", None),                      # Large value: terabytes, but ValueError is expected so None here to not assert value
        ("", None),                           # Edge: empty string; IndexError or ValueError expected so None here to not assert value
        ("-10K", -10 * 1024),                 # Edge: negative value kilobytes
    ],
)
def test_memstr_to_bytes(text, value):
    # For inputs that should raise, just test raising here
    if text in ("1024T", ""):
        try:
