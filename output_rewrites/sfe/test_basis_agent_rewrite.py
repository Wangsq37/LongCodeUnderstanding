...
def test_oriented_saveload(m: Mesh):
    ...
    m = m.refined(4)
    m = m.with_boundaries({"edge_test": m.facets_around([0])})
    assert len(m.boundaries["edge_test"].ori) == m.refdom.nfacets

    M = from_meshio(to_meshio(m))

    assert_array_equal(
        m.boundaries["edge_test"].ori, M.boundaries["edge_test"].ori,
    )
...
def test_with_elements():
    mesh = MeshQuad().refined(2).with_subdomains({'left': lambda x: x[0] < 0.25})
    basis = CellBasis(mesh, ElementQuad0())
    basis_half = basis.with_elements('left')
    ...
    assert all(basis_half.tind == basis.mesh.normalize_elements('left'))


def test_with_elements():
    mesh = MeshHex().refined(1).with_subdomains({'front': lambda x: x[2] < 0.5})
    basis = CellBasis(mesh, ElementHex0())
    basis_half = basis.with_elements('front')

    assert basis.mesh == basis_half.mesh
    assert basis.elem == basis_half.elem
    assert basis.mapping == basis_half.mapping
    assert basis.quadrature == basis_half.quadrature
    assert all(basis_half.tind == basis.mesh.normalize_elements('front'))


def test_with_elements():
    mesh = MeshTet().refined(3).with_subdomains({'lowz': lambda x: x[2] < 0.3})
    basis = CellBasis(mesh, ElementTetP0())
    basis_half = basis.with_elements('lowz')

    assert basis.mesh == basis_half.mesh
    assert basis.elem == basis_half.elem
    assert basis.mapping == basis_half.mapping
    assert basis.quadrature == basis_half.quadrature
    assert all(basis_half.tind == basis.mesh.normalize_elements('lowz'))


def test_with_elements():
    mesh = MeshLine1().refined(2).with_subdomains({'mid': lambda x: (x[0] > 0.5) & (x[0] < 0.75)})
    basis = CellBasis(mesh, ElementLineP0())
    basis_half = basis.with_elements('mid')

    assert basis.mesh == basis_half.mesh
    assert basis.elem == basis_half.elem
    assert basis.mapping == basis_half.mapping
    assert basis.quadrature == basis_half.quadrature
    assert all(basis_half.tind == basis.mesh.normalize_elements('mid'))
