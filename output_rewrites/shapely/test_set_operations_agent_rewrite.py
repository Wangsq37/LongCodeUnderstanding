...
def test_set_operation_array(a, func):
    # NEW: Use empty geometry as input and compare with normal geometry
    actual = func(empty, a)
...
def test_set_operation_prec_array(a, func, grid_size):
    # NEW: Use negative and large grid_size for effective edge cases
    if func is shapely.disjoint_subset_union:
        pytest.skip("disjoint_subset_union does not support grid_size")
    # Use reversed order for testing as well
...
def test_set_operation_reduce_axis(func, related_func, n):
    # NEW: Use array of different polygons and empty in 2D shapes, and axis param
    data = np.array(
        [
            [reduce_test_data[0], reduce_test_data[1]],
            [reduce_test_data[2], empty],
            [reduce_test_data[3], reduce_test_data[1]],
        ],
        dtype=object,
    )  # shape = (3, 2)
    # Test reduce with axis=None
    actual = func(data, axis=None)
    assert isinstance(actual, Geometry)
    # axis=0: should reduce over rows (get length 2)
    actual0 = func(data, axis=0)
    assert actual0.shape == (2,)
    # axis=1: over columns
    actual1 = func(data, axis=1)
    assert actual1.shape == (3,)
    # axis=-1: identical to axis=1
    actualm1 = func(data, axis=-1)
    assert actualm1.shape == (3,)


@pytest.mark.parametrize("func, related_func", REDUCE_SET_OPERATIONS)
def test_set_operation_reduce_empty(func, related_func):
    # NEW: Test with empty input, 3D and also all None
...
def test_set_operation_reduce_some_none_len2(func, related_func):
    # NEW: Reverse order and both None/empty
...
def test_set_operation_reduce_all_none_arr(n, func, related_func):
    # NEW: Use n=3 and n=4 for larger all-None arrays
...
def test_set_operation_prec_reduce_axis(func, related_func):
    # NEW: use different points and larger 2d data
    data = [
        [point, empty],
        [shapely.Point(10, 10), shapely.Point(-10, -10)],
        [shapely.Point(5, 5), point]
    ]  # shape = (3, 2)
    actual = func(data, grid_size=10, axis=None)  # default
    assert isinstance(actual, Geometry)
...
def test_coverage_union_reduce_axis(n):
    # NEW: Use more complex polygons and array shape
