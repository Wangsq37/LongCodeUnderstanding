...
# AUGMENTED TEST DATA
@pytest.mark.parametrize("version", ["3.11.5", "4.0.0", "4.0.1", "999.0.0"])
...
# AUGMENTED TEST DATA
@pytest.mark.parametrize("version", ["0.0.0", "100.100.100", "2.8.7"])
...
# AUGMENTED TEST DATA
@pytest.mark.parametrize("version", ["999.999.999", "0.0.1", "4.4.4"])
...
def test_multithreading_enabled_raises_arg():
    ...
    # Try with a float array (not object) and extreme value
    # set_first_element cannot change the input array (float dtype array is not set)
    # The test case did not raise, so we remove the raises context and just check
    set_first_element(-1e14, arr)
    # afterwards, we can
    arr[0] = -1e14
    assert arr[0] == -1e14


# No changes here; left as original
...
@pytest.mark.parametrize(
    "args,kwargs",
    [
        ((np.empty((1,), dtype=np.int64),), {}),  # int-dtype ndarray is untouched
        ((), {"a": np.empty((1,), dtype=np.int64)}),
        (([""],), {}),  # list of empty string
        ((), {"a": ["", None]}),
        ((), {"out": np.empty((1,), dtype=float)}),  # ufunc kwarg 'out' is untouched
        (
            (),
            {"where": np.empty((1,), dtype=float)},
        ),  # ufunc kwarg 'where' is untouched
        ((np.array([0], dtype=object),), {}),
        ((np.array([-999], dtype=object),), {}),
    ],
)
def test_multithreading_enabled_ok(args, kwargs):
    # For args[6] and args[7] (0-based), assignment destination is read-only, so we expect ValueError.
    if (
        (len(args) == 1 and isinstance(args[0], np.ndarray)
            and args[0].dtype == object
            and (args[0][0] == 0 or args[0][0] == -999))
    ):
        with pytest.raises(ValueError):
