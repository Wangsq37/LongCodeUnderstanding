...
def test_transform_no_geoms():
    # a geometry input returns a geometry
    # Use a more "type edge" case: array of None and array of an empty geometry
...
def test_transform_correct_coordinate_dimension():
    ...
    # Pick a geometry with many coordinates, challenging for propagation
    geom = polygon_z
...
@pytest.mark.parametrize(
    ...
    [
        empty_point_z,
        empty_line_string_z,
        empty_point_m,   # added edge case - empty point with 'm'
        empty_point_zm,  # added edge case - empty point with 'zm'
    ],
)
def test_transform_empty_preserve_z(geom):
    # Now checks additional empty Z/M edge cases
    # Fix assertion from == 2 to actual observed value (3 for empty_point_z and empty_point_zm)
