...
def test_points_nan_all_nan_becomes_empty():
    # augment: test zero and negative nan values
    actual = shapely.points(np.nan, -np.inf)
    assert actual.wkt == "POINT (NaN -Infinity)"
...
def test_points_nan_3D_all_nan_becomes_empty_2D():
    # augment: mix nan and inf and a large negative value
...
def test_points_nan_3D_all_nan_becomes_empty():
    # augment: all negative infinite values in 3D
    actual = shapely.points(-np.inf, -np.inf, -np.inf)
    assert actual.wkt == "POINT Z EMPTY"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([1e9, np.nan], "POINT (1000000000 NaN)"),
        ([np.nan, -1e9], "POINT (NaN -1000000000)"),
        ([np.nan, 1, np.nan], "POINT Z (NaN 1 NaN)"),
        ([np.nan, np.nan, -1e7], "POINT Z (NaN NaN -10000000)"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([-999, np.nan], "skip", "POINT EMPTY"),
        ([np.nan, -100], "skip", "POINT EMPTY"),
    ],
)
...
def test_linestrings_from_coords():
    actual = shapely.linestrings([[[0, 0], [100, 100]], [[-50, 0], [2, 2]]])
...
def test_linestrings_from_xy():
    actual = shapely.linestrings([2, -3], [0, 1])
    assert_geometries_equal(actual, LineString([(2, 0), (-3, 1)]))


def test_linestrings_from_xy_broadcast():
    x = [0, 1]  # the same X coordinates for both linestrings
    y = [2, -3], [4, 100]  # each linestring has a different set of Y coordinates
    actual = shapely.linestrings(x, y)
    assert_geometries_equal(
        actual,
        [
            LineString([(0, 2), (1, -3)]),
            LineString([(0, 4), (1, 100)]),
        ],
    )


def test_linestrings_from_xyz():
    actual = shapely.linestrings([0, 1], [2, 3], -10)
    assert_geometries_equal(actual, LineString([(0, 2, -10), (1, 3, -10)]))
...
def test_polygon_with_1_hole():
    # augment: make outer box bigger and hole bigger
    actual = shapely.polygons(box_tpl(0, 0, 100, 100), [box_tpl(10, 10, 30, 30)])
    # box area is 100*100=10000; hole area 20*20=400
    assert shapely.area(actual) == 9600.0


def test_polygon_with_2_holes():
    # augment: change holes and outer box
    actual = shapely.polygons(
        box_tpl(0, 0, 20, 20), [box_tpl(2, 2, 8, 8), box_tpl(10, 10, 12, 12)]
    )
    # outer box area: 20*20=400, first hole: 6*6=36, second: 2*2=4
    assert shapely.area(actual) == 360.0


def test_polygon_with_none_hole():
    # augment: test none/interleaved None and valid holes, larger box
    actual = shapely.polygons(
        shapely.linearrings(box_tpl(0, 0, 100, 100)),
        [
            None,
            shapely.linearrings(box_tpl(1, 1, 10, 10)),
            None,
            shapely.linearrings(box_tpl(90, 90, 98, 98)),
        ],
    )
    # outer: 100*100=10000; holes: 9*9=81, 8*8=64; total: 10000-81-64 = 9855
    assert shapely.area(actual) == 9855.0


def test_2_polygons_with_same_hole():
    # augment: larger and negative coordinates
    actual = shapely.polygons(
        [box_tpl(0, 0, 50, 50), box_tpl(-20, -20, 5, 5)], [box_tpl(1, 1, 2, 2)]
    )
    # first polygon 50*50=2500-1=2499; second 25*25=625-1=624 (hole 1*1=1)
    assert shapely.area(actual).tolist() == [2499.0, 624.0]


def test_2_polygons_with_2_same_holes():
    # augment: larger boxes, different holes
    actual = shapely.polygons(
        [box_tpl(0, 0, 20, 20), box_tpl(-10, -10, 10, 10)],
        [box_tpl(5, 5, 6, 6), box_tpl(7, 7, 8, 8)],
    )
    # first outer: 400, hole: 1*1=1; second outer: 20*20=400, hole 1*1=1
    assert shapely.area(actual).tolist() == [398.0, 398.0]


def test_2_polygons_with_different_holes():
    # augment: switch inner holes to cover more complex areas
    actual = shapely.polygons(
        [box_tpl(0, 0, 100, 100), box_tpl(0, 0, 20, 20)],
        [[box_tpl(10, 10, 20, 20)], [box_tpl(2, 2, 4, 4)]],
    )
    # first outer: 10000 hole: 10*10=100, second outer: 400 hole: 2*2=4
    assert shapely.area(actual).tolist() == [9900.0, 396.0]
...
def test_prepare():
    # augment: test with large values, negative, and 3D points
    arr = np.array([
        shapely.points(-1000, 1e8),
        None,
        shapely.box(-10, -10, 10, 10),
        shapely.points(0, 0, 9999)
    ])
