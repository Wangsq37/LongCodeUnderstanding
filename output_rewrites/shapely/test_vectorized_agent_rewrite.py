...
class VectorizedContainsTestCase(unittest.TestCase):
    ...
    def test_array_2d(self):
        # Augmented: more diverse grid, including edge cases like zeros, negatives, large floats
        y, x = np.mgrid[-20:20:11j, -10:35:10j]  # y is shape (11,10), x is shape (11,10)
        # Add in a float grid to check dtype handling
        x = x.astype(np.float64) * 1.5  # amplify values, convert to float
        y = y.astype(np.float64) * 2.0

        result = self.assertContainsResults(self.construct_torus(), x, y)
        assert result.shape == x.shape  # Expect shape to match: (11,10)
