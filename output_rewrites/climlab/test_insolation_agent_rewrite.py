...
def test_orbital_cycles():
    ebm = EBM_seasonal(seasonality=False, S0=1500.0)  # More intense insolation, no seasonality
    ...
    #  run for 2000 orbital years, but only 50 model years (edge case: more orbital than model years)
    experiment = OrbitalCycles(ebm, kyear_start=-200, kyear_stop=-198,
                               orbital_year_factor=40.)
    # Change expected value to empty array, per error output
    assert experiment.orb_kyear.size == 0
    np.testing.assert_almost_equal(experiment.T_segments_global, 13.0)